...
- **音频分析**: Web Audio API
- **封面渲染**: PixiJS 7.4.3
- **本地元数据**: better-sqlite3 + music-metadata
...
- 支持多目录导入与自动扫描
- 支持 `MP3 / FLAC / M4A / AAC / OGG / WAV` 扫描格式筛选
- 支持歌曲 / 专辑 / 歌手三视图浏览
- 支持虚拟滚动与增量扫描，降低大库二次扫描开销
- 支持定位到当前歌曲文件所在位置
- 支持本地删除（仅删索引）与彻底删除（删文件与同名 `.lrc`）
- 支持扫描时预读取内嵌歌词与同名 `.lrc`
- 支持本地歌曲播放时在线补齐歌词与封面，并写回本地
...
- **eqEnabled**: 均衡器启用状态
- **localLibraryRoots**: 本地乐库目录列表
- **localLibraryScanFormats**: 本地乐库扫描格式
- **localLibraryOnlineLyricMatchEnabled**: 在线补齐本地歌词与封面
- **showLocalLibraryMenu**: 是否显示本地乐库入口
